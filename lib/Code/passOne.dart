...
class passOne {
  ...
  passOne(List data) {
    ...
    try {
      util.Lines.forEach((element) {
        if (element.instruction_normal.toLowerCase() == "end" &&
            int.parse(element.operands_normal.trim().toString(), radix: 16) !=
                util.startingAddress) {
          print(
              "saaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaad   ${element.toString()}");
      ...
      if (int.tryParse(util.Lines.last.operands_normal, radix: 16) !=
          util.startingAddress)
        util.errorsTable.add("your start position of your program not found ");
    } catch (e) {
      if (util.Lines.last.instruction_normal.toUpperCase() != "END") {
        util.errorsTable.add(
            "your end position of your program not found  ${util.Lines.last.toString()} ${util.Lines.last.instruction_normal.toUpperCase()} != END");
      } else {
    ...
    util.Lines.forEach((element) {
      ...
      if (element.type) {
        ...
        switch (element.instruction_normal) {
          case "resb":
            currentAddress += int.parse(element.operands_normal);
            break; //7nzwed 3add el bit dy 3ad
          case "resw":
            currentAddress += 3 * int.parse(element.operands_normal);
            break; // 3shan de ward
          case "byte":
            int s = element.operands_normal
                .substring(2, element.operands_normal.length - 1)
                .length;
            print(
                "ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss    $s    ${element.operands_normal.substring(2, element.operands_normal.length - 1)}");
            currentAddress +=
                element.operands_normal.toString().toLowerCase().indexOf('x') ==
                        0
                    ? s / 2
                    : s;
            break;
